...
TWO_HAND_FEATURES_DIR = DATASET_DIR / "two_hand_features"
...
COMPOUND_VOWEL_LABEL_IDS = {28, 29, 33, 34}
TWO_HAND_LABEL_IDS = set()
DOUBLE_CONSONANT_LABEL_IDS = {1, 4, 8, 10, 13}
EXCLUDED_COMPOSED_LABEL_IDS = DOUBLE_CONSONANT_LABEL_IDS | COMPOUND_VOWEL_LABEL_IDS
...
def build_default_config():
    ...
    for index, (code, name, group, assignee) in enumerate(DEFAULT_LABEL_SPECS):
        ...
        if index in TWO_HAND_LABEL_IDS:
            label.update(
                {
                    "description": f"양손 {group} 정적 수화 ({code})",
                    "hand_requirement": "both",
                    "collection_mode": "both_hands",
                }
            )
        if index in COMPOUND_VOWEL_LABEL_IDS:
            pairs = {28: "ㅗ+ㅏ", 29: "ㅗ+ㅐ", 33: "ㅜ+ㅓ", 34: "ㅜ+ㅔ"}
            label["description"] = f"조합 전용 겹모음 ({pairs[index]})"
        labels.append(label)
    ...
    return {
        ...
        "feature_dim": 83,
        "active_label_ids": [
            label_id for label_id in range(40) if label_id not in EXCLUDED_COMPOSED_LABEL_IDS
        ] + [42],
        "two_hand_label_ids": sorted(TWO_HAND_LABEL_IDS),
        "collection_label_ids": [
            label_id for label_id in range(43) if label_id not in EXCLUDED_COMPOSED_LABEL_IDS
        ],
...
class DatasetHandler(BaseHTTPRequestHandler):
    ...
    def handle_start_session(self, payload):
        ...
        if label.get("runtime_rule"):
            save_dir = CONTROL_FEATURES_DIR
            dataset_type = "control_features"
        elif collection_mode == "both_hands":
            save_dir = TWO_HAND_FEATURES_DIR
            dataset_type = "two_hand_features"
        else:
